import { getAsync } from '@util/ref/redis.utils'
import { AlignmentObjectExtended } from '@/models/ref/alignment-object-extended'
import { FilterOption, FilterOptionChild } from '@/models/ref/filter-option'
...
async function getPerusopetuksenOppiaineet(lang: string): Promise<FilterOption> {
  try {
    const subjects: AlignmentObjectExtended[] = JSON.parse(
      await getAsync(`${rediskeyBasic}.${lang}`)
    )
...
async function getLukionOppiaineet(lang: string): Promise<FilterOption> {
  try {
    const subjects: AlignmentObjectExtended[] = JSON.parse(
      await getAsync(`${rediskeyUpperSecondary}.${lang}`)
    )
...
async function getAmmattikoulunTutkinnot(lang: string): Promise<FilterOption> {
  try {
    const degrees: AlignmentObjectExtended[] = JSON.parse(
      await getAsync(`${rediskeyVocational}.${lang}`)
    )
...
async function getTieteenalat(lang: string): Promise<FilterOption> {
  try {
    const branches: AlignmentObjectExtended[] = JSON.parse(
      await getAsync(`${rediskeyHigher}.${lang}`)
    )
