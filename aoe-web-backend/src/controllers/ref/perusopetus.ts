...
import { sortByTargetName } from '@util/ref/data.utils'
...
export async function setPerusopetuksenOppiaineet(): Promise<void> {
    ...
    ;(result as any).vuosiluokkakokonaisuudet.forEach((gradeEntity: any) => {
      if (gradeEntity.tavoitteet?.length > 0) {
        gradeEntity.tavoitteet.forEach((objective: any) => {
          if (
            !objective.id ||
            (!objective.tavoite?.fi && !objective.tavoite?.sv) ||
            !gradeEntity.id
          ) {
            throw Error(
      ...
      if (gradeEntity.sisaltoalueet?.length > 0) {
        gradeEntity.sisaltoalueet.forEach((content: any) => {
          if (!content.id || (!content.nimi?.fi && !content.nimi?.sv) || !gradeEntity.id) {
            throw Error(
              'Creating new sets of contents failed in setPerusopetuksenOppiaineet(): Missing required data'
            )
          }
  ...
  if (!competences) {
    winstonLogger.error(
      'No data from ePerusteet for transversal competences in setPerusopetuksenOppiaineet()'
    )
    return
  }
  ...
  try {
    ...
    if (
      finnishCompetences.length <
        JSON.parse(await getAsync(`${rediskeyTransversalCompetences}.fi`))?.length ||
      swedishCompetences.length <
        JSON.parse(await getAsync(`${rediskeyTransversalCompetences}.sv`))?.length
    ) {
      winstonLogger.error(
        'Creating new sets of transversal competences failed in setPerusopetuksenOppiaineet()'
      )
      return
    }
