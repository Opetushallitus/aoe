...
export async function aoeResponseMapper(response: ApiResponse<SearchResponse<Source>>) {
  try {
    ...
    if (hits) {
      ...
      if (source) {
        resp.results = source.map((obj) => {
          ...
          rObj.materialName = mname
            ? mname.map((name) => ({
                materialname: name.materialname,
                language: name.language
              }))
            : undefined
          rObj.description = obj.materialdescription
            ? obj.materialdescription.map((description) => ({
                description: description.description,
                language: description.language
              }))
            : undefined
          rObj.authors = obj.author
            ? obj.author.map((author) => ({
                authorname: author.authorname,
                organization: author.organization,
                organizationkey: author.organizationkey
              }))
            : undefined
          rObj.learningResourceTypes = obj.learningresourcetype
            ? obj.learningresourcetype.map((lrt) => ({
                value: lrt.value,
                learningresourcetypekey: lrt.learningresourcetypekey
              }))
            : undefined
          rObj.license = { key: obj.license.key, value: obj.license.value }
          ;(rObj.educationalLevels = obj.educationallevel
            ? obj.educationallevel.map((el) => ({
                value: el.value,
                educationallevelkey: el.educationallevelkey
              }))
            : undefined),
            (rObj.educationalRoles = obj.educationalaudience
              ? obj.educationalaudience.map((role) => ({
                  value: role.educationalrole,
                  educationalrolekey: role.educationalrolekey
                }))
              : undefined),
            (rObj.keywords = obj.keyword
              ? obj.keyword.map((word) => ({
                  value: word.value,
                  keywordkey: word.keywordkey
                }))
              : undefined),
            (rObj.languages = obj.materials
              ? Array.from(new Set(obj.materials.map((material) => material.language)))
              : undefined),
            (rObj.educationalSubjects = obj.alignmentobject
              ? obj.alignmentobject
                  .filter((object) => {
              ...
              : undefined),
            (rObj.teaches = obj.alignmentobject
              ? obj.alignmentobject
                  .filter((object) => {
          ...
          rObj.educationalUses = obj.educationaluse
            ? obj.educationaluse.map((element) => ({
                value: element.value,
                educationalusekey: element.educationalusekey
              }))
            : undefined
          rObj.accessibilityFeatures = obj.accessibilityfeature
            ? obj.accessibilityfeature.map((element) => ({
                value: element.value,
                accessibilityfeaturekey: element.accessibilityfeaturekey
              }))
            : undefined
          rObj.accessibilityHazards = obj.accessibilityhazard
            ? obj.accessibilityhazard.map((element) => ({
                value: element.value,
    ...
    log.error('Elasticsearch query failed', err)
    throw err
  }
}
...
export function hasDownloadableFiles(materials: Array<{ filekey: string }>) {
  try {
    ...
    log.error('Error in hasDownloadableFiles()', err)
    throw err
  }
}
...
export const elasticSearchQuery = async (
  ...
  try {
    ...
    const result: ApiResponse<SearchResponse<Source>> =
      await client.search<SearchResponse<Source>>(query)
...
export function filterMapper(filters: AoeRequestFilter) {
  try {
    ...
    if (filters.educationalLevels) {
      createShouldObject(
        filter,
        'educationallevel.educationallevelkey.keyword',
    ...
    if (filters.learningResourceTypes) {
      createShouldObject(
        filter,
        'learningresourcetype.learningresourcetypekey.keyword',
        filters.learningResourceTypes
      )
    }
    if (filters.educationalSubjects) {
      createShouldObject(
        filter,
        'alignmentobject.objectkey.keyword',
        filters.educationalSubjects,
        'educationalSubject'
      )
    }
    if (filters.educationalRoles) {
      createShouldObject(
        filter,
        'educationalaudience.educationalrolekey.keyword',
        filters.educationalRoles
      )
    }
    if (filters.authors) {
    ...
    if (filters.educationalUses) {
      createShouldObject(
        filter,
        'educationaluse.educationalusekey.keyword',
        filters.educationalUses
      )
    }
    if (filters.accessibilityFeatures) {
      createShouldObject(
        filter,
        'accessibilityfeature.accessibilityfeaturekey.keyword',
    ...
    if (filters.accessibilityHazards) {
      createShouldObject(
        filter,
        'accessibilityhazard.accessibilityhazardkey.keyword',
        filters.accessibilityHazards
      )
    }
    if (filters.licenses) {
 ...
 */
export function createShouldObject(
  filter: Array<any>,
  key: string,
  valueList: Array<string>,
  alignmentObjectType?: string
) {
  try {
