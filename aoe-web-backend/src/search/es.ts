...
import {
  collectionDataToEs,
  collectionFromEs,
  getCollectionDataToEs,
  getCollectionDataToUpdate
} from './esCollection'
    ...
    db.tx({ mode }, async (t: any) => {
      ...
      return t
        .map(query, params, async (q: any) => {
          const m: any = []
          t.map(
            'select m.id, m.materiallanguagekey as language, link, version.priority, filepath, originalfilename, filesize, mimetype, filekey, filebucket, obsoleted ' +
              ...
              'left join material m on m.id = version.materialid left join record r on m.id = r.materialid where m.educationalmaterialid = $1',
            [q.id],
            (q2: any) => {
      ...
      .then(async (data: any) => {
        if (data.length > 0) {
          ...
          const { statusCode, body: bulkResponse } = await performBulkOperation(
            client,
            indexName,
            body
          )
        ...
        .map(query, params, async (q: any) => {
          ...
          t.map(
            'SELECT m.id, m.materiallanguagekey AS language, link, version.priority, filepath, ' +
              ...
              'WHERE m.educationalmaterialid = $1',
            [q.id],
            (q2: any) => {
              t.any('select * from materialdisplayname where materialid = $1;', q2.id).then(
                (data: any) => {
          ...
          query =
            'SELECT licensecode AS key, license AS value ' +
