 ...
 */
async function aoeCollectionResponseMapper(
  response: ApiResponse<SearchResponse<AoeCollectionResult>>
) {
  try {
...
export const getCollectionDataToEs = async (offset: number, limit: number) => {
  try {
    return await db.tx(async (t: any) => {
      let query =
        'select collection.id, publishedat, updatedat, createdat, collectionname as name, description from collection WHERE publishedat IS NOT NULL order by collection.id asc OFFSET $1 LIMIT $2;'
      const params: any = []
    ...
    log.error('Failed to get collection data from database', err)
    throw err
  }
}
...
export async function collectionDataToEs(index: string, data: any, operation: 'create' | 'index') {
  try {
    if (data.length > 0) {
      ...
      log.info(`Adding ${data.length} documents to OpenSearch index ${index}`)
      ...
      if (bulkResponse.errors) {
        ...
        log.error('Document errors', erroredDocuments)
      }
    }
  } catch (err) {
    log.error(`Failed to add documents to OpenSearch index ${index}`, err)
    throw err
  }
}
...
export const getCollectionDataToUpdate = async (time: Date) => {
  try {
    return await db.tx(async (t: any) => {
      let query =
        'select collection.id, publishedat, updatedat, createdat, collectionname as name, description from collection WHERE updatedat > $1 and publishedat IS NOT NULL;'
      const params: any = []
      params.push(time)
      const collections = await Promise.all(
        await t.map(query, params, async (q: any) => {
    ...
    log.error('Failed to get collection data to update from database', err)
    throw err
  }
}
