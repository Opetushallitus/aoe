...
test('materiaalin arvostelu ja arvostelujen näkyvyys kirjautumattomalle käyttäjälle', async ({
  ...
  const materiaaliNimi = uusiMateriaali.randomMateriaaliNimi('Arvosteltava materiaali')
  ...
  // Set language to Finnish
  await anonPage.getByRole('button', { name: 'Suomi: Vaihda kieli suomeksi' }).click()
  ...
  await expect(arvostelu.getByText('Erittäin hyvä ja selkeä materiaali')).toBeVisible()
  await expect(arvostelu.getByText('Voisi olla enemmän esimerkkejä')).toBeVisible()
