...
test.describe.serial('haku', () => {
  ...
  test('luo oppimateriaali ja odota indeksointia', async ({ page }) => {
    ...
    const hakuTulos = page.locator('article.search-result h1 a', { hasText: materiaaliNimi })
    await expect(async () => {
      await etusivu.goto()
  ...
  test('käyttäjä voi etsiä oppimateriaalia oppimateriaalin tyypin perusteella', async ({ page }) => {
    const etusivu = Etusivu(page)
    await etusivu.goto()
