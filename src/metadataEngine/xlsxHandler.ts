...
async function validate(data: any) {
  ...
  for (const d in data) {
    ...
    if (data[d].julkaisuajankohta === undefined) {
      ...
      if (da.length !== 10) {
        o[key].push(
          createMessage(row, 'julkaisuajankohta', 'Field must contain 8 characters. Correct format is yyyymmd'),
        );
    ...
    const saavutettavuudenTukitoiminnotList: any = await createPropertyNameList(
      data[d],
      'saavutettavuuden_tukitoiminnot',
    );
    if (await hasDuplicates(saavutettavuudenTukitoiminnotList, data[d])) {
      o[key].push(
        createMessage(row, 'saavutettavuuden_tukitoiminnot', 'saavutettavuuden_tukitoiminnot has dublicate value'),
      );
    }
