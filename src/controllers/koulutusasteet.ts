...
export async function setKoulutusasteet(): Promise<void> {
  ...
  data.forEach((row: any) => {
    if (!row.key || (!row.value?.fi && !row.value?.sv && !row.value?.en)) {
      throw Error('Creating new sets of educational levels failed in setKoulutusasteet(): Missing required data');
    }
 ...
 */
export const getKoulutusasteet = async (req: Request, res: Response, next: NextFunction): Promise<EducationLevel[]> => {
  try {
 ...
 * @returns {Promise<EducationLevel>}
 */
export const getKoulutusaste = async (req: Request, res: Response, next: NextFunction): Promise<EducationLevel> => {
  try {
    const redisData: string = await getAsync(`${rediskey}.${req.params.lang.toLowerCase()}`);
