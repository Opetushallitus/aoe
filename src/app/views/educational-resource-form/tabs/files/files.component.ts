...
export class FilesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptionLanguage = this.koodistoService.languages$.subscribe((languages: Language[]): void => {
      this.languages = languages;
    });
    this.subscriptionUploadedFiles = this.uploadedFiles$.subscribe((uploadedFiles: UploadedFile[]): void => {
  ...
  ngOnDestroy(): void {
    if (this.submitted === false && this.form.dirty && this.form.valid) this.saveData();
  ...
  updateLanguages(): void {
    ...
    this.otherLangs = this.translate.getLangs().filter((lang: string): boolean => lang !== this.lang);
  }
      ...
      if (subtitles.value.length > 0) {
        subtitles.controls.forEach((subCtrl: AbstractControl): void => {
          if (!subCtrl.get('file').value) {
            subtitles.removeAt(subtitles.controls.findIndex((sub: AbstractControl): boolean => sub === subCtrl));
          }
        });
          ...
          if (res.results.response && res.metadata.subtitles.length > 0) {
            res.metadata.subtitles.forEach((subtitle): void => {
              ...
              subtitleUploads.push(
                this.materialService.uploadSubtitle(res.results.response.material[0].id, subFormData),
              );
            });
          }
        });
          ...
          forkJoin(subtitleUploads).subscribe(
            (attachments: { id: string }[]): void => {
              attachments.forEach((attachment: { id: string }): void => {
                if (attachment.id) this.completedUploads++;
              });
              this.materialService.updateUploadedFiles(this.materialService.getEducationalMaterialID()).subscribe({
                error: (err) => console.error('Update of uploaded files failed in the files tab:', err),
  ...
  completeUpload(): void {
    ...
    if (this.totalFileCount === 0) this.calculateTotalFileCount();
    this.submitted = false;
    this.uploadActive = false;
    ...
    this.materialService.setMaterialObsoleted(fileId).subscribe({
      ...
      complete: (): void => {
        this.materialService.updateUploadedFiles(this.educationalMaterialID).subscribe({
          error: (err): void => console.error(`Updating the uploaded files failed in file deletion: ${err}`),
        });
      },
    });
  }
    ...
    this.materialService
      ...
      .subscribe({
        ...
        complete: () =>
          this.materialService.updateUploadedFiles(this.materialService.getEducationalMaterialID()).subscribe({
            error: (err): void =>
              console.error(`Updating the uploaded files failed after setting an attachment obsoleted: ${err}`),
          }),
      });
  }
        ...
        if (this.totalFileCount > 0) {
          ...
          if (!this.materialService.getEducationalMaterialID()) {
            ...
            this.materialService.createEmptyEducationalMaterial(formData).subscribe(
              ...
              (err) => console.error('Create empty framework request failed in the file upload:', err),
              () => this.uploadFiles(),
            );
          } else {
            this.uploadFiles();
