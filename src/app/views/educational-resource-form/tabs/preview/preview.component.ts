...
export class PreviewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.setTitle();
    this.subscriptionLangChange = this.translate.onLangChange.subscribe((event: LangChangeEvent): void => {
    ...
    if (this.upperSecondarySchoolSubjectsNew?.length) {
      ...
      const ignoredSubjectsList: AlignmentObjectExtended[] = this.upperSecondarySchoolSubjectsNew.filter(
  ...
  drop(event: CdkDragDrop<UploadedFile[]>): void {
    moveItemInArray(this.materialService.getUploadedFiles(), event.previousIndex, event.currentIndex);
  }
  ...
  onSubmit(): void {
    if (this.form.valid) {
      ...
      this.materialService.updateEducationalMaterialMetadata(this.educationalMaterialID, this.savedData).subscribe({
        error: (error): void => {
