...
const storage = multer.diskStorage({
  ...
  filename: function(req: Request, file: any, cb: any) {
    ...
    let str = file.originalname.substring(
      0,
      file.originalname.lastIndexOf(".")
    );
 ...
 */
export async function uploadAttachmentToMaterial(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<any> {
      ...
      upload.single("attachment")(req, res, async function(err: any) {
        try {
          ...
          if (typeof file !== "undefined") {
            ...
            result = await insertDataToTempAttachmentTable(
              file,
              metadata,
              attachmentId
            );
          ...
          try {
            if (typeof file !== "undefined") {
              ...
              await deleteDataToTempAttachmentTable(
                file.filename,
                result[0].id
              );
            ...
            winstonLogger.error(
              "error while sending files to pouta: " +
                error +
                " - " +
          ...
          if (!file) {
            await db
              .tx(async (t: any) => {
                ...
                if (req.body.name) {
                  await insertEducationalMaterialName(
            ...
            db.tx(async (t: any) => {
              ...
              materialid = id.id;
              let result = await insertDataToDisplayName(
                t,
                emresp.id,
                id.id,
                fileDetails
                ...
                try {
                  if (typeof file !== "undefined") {
                    ...
                    try {
                      ...
                      winstonLogger.debug(
                        "ERROR converting office file to pdf"
                      );
                      winstonLogger.error(e);
                    }
                    await deleteDataFromTempRecordTable(
                      file.filename,
                      materialid
                    );
                    fs.unlink("./" + file.path, (err: any) => {
                  ...
                  winstonLogger.debug(
                    "error while sending file to pouta: " +
                      JSON.stringify((<any>req).file)
                  );
                }
              })
 ...
 */
export async function uploadFileToMaterial(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<any> {
      ...
      upload.single("file")(req, res, async function(err: any) {
        try {
          ...
          if (!file) {
            ...
            winstonLogger.debug(
              "uploadFileToMaterial details to database for: " +
                file.originalname
            );
            let materialid: string;
            const fileDetails = JSON.parse(req.body.fileDetails);
            ...
            db.tx(async (t: any) => {
              ...
              .then(async (data: any) => {
                // return 200 if success and continue sending files to pouta
                winstonLogger.debug(
                  "uploadFileToMaterial sending to Pouta: " + file.filename
                );
                ...
                try {
                  if (typeof file !== "undefined") {
                    ...
                    try {
                      ...
                      winstonLogger.error(
                        "ERROR converting office file to pdf: " + e
                      );
                    }
                    await deleteDataFromTempRecordTable(
                      file.filename,
              ...
              .catch((err: Error) => {
                ...
                fs.unlink("./" + file.path, (err: any) => {
                  if (err) {
                    winstonLogger.error(
                      "Error in uploadFileToMaterial(): " + err
                    );
                  } else {
                    winstonLogger.debug("file removed");
                  }
 ...
 */
export async function fileToStorage(
  file: any,
  materialid: string
...
export async function checkTemporaryRecordQueue(): Promise<any> {
  try {
    ...
    const query =
      "Select * From temporaryrecord where extract(epoch from createdat)*1000 < $1 limit 1000;";
...
export async function checkTemporaryAttachmentQueue(): Promise<any> {
  try {
    ...
    const ts = Date.now() - 1000 * 60 * 60;
    const query =
      "Select * From temporaryattachment where extract(epoch from createdat)*1000 < $1 limit 1000;";
    const data = await db.any(query, [ts]);
    for (const element of data) {
      const metadata = {
      ...
      try {
        ...
        winstonLogger.error(
          "Error in checkTemporaryAttachmentQueue(): " + error
        );
      }
    }
  } catch (error) {
...
export async function insertDataToEducationalMaterialTable(
  req: Request,
  t: any
): Promise<any> {
  const query =
...
export async function updateAttachment(
  ...
  await db
    .tx(async (t: any) => {
      ...
      winstonLogger.debug(query);
      queries.push(
        await db.none(query, [location, fileKey, fileBucket, attachmentId])
      );
      return t.batch(queries);
    })
    .catch((err: Error) => {
...
export async function insertDataToTempAttachmentTable(
  files: any,
  metadata: any,
  attachmentId: string
): Promise<any> {
...
export async function insertDataToTempRecordTable(
  t: any,
  files: any,
  materialId: any
): Promise<any> {
...
export async function deleteDataFromTempRecordTable(
  filename: any,
  materialId: any
): Promise<any> {
  const query =
    "DELETE FROM temporaryrecord WHERE filename = $1 AND materialid = $2";
  winstonLogger.debug(query);
  return await db.any(query, [filename, materialId]);
}

export async function deleteDataToTempAttachmentTable(
  filename: any,
  materialId: any
): Promise<any> {
  const query =
    "DELETE FROM temporaryattachment WHERE filename = $1 AND id = $2";
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      fs.readFile(filePath, async (err: any, data: any) => {
        if (err) {
          winstonLogger.error(
            "Reading file from the local file system failed in uploadFileToStorage(): " +
              err
          );
        ...
        try {
          ...
          s3.upload(params, (err: any, data: any) => {
            if (err) {
              winstonLogger.error(
                "Uploading file to the cloud object storage failed in uploadFileToStorage(): " +
                  err
              );
          ...
          winstonLogger.error(
            "Error in uploading file to the cloud object storage in uploadFileToStorage(): " +
              err
          );
          reject(new Error(err));
        }
      ...
      winstonLogger.error(
        "Error in processing file in uploadFileToStorage(): " + err
      );
      reject(new Error(err));
    }
  });
...
export async function uploadBase64FileToStorage(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      winstonLogger.error(
        "Error in processing file in uploadBase64FileToStorage(): " + err
      );
      reject(new Error(err));
    }
  });
...
export const downloadPreviewFile = async (
  ...
  winstonLogger.debug(
    "HTTP request headers present in downloadPreviewFile(): %o",
    req.headers
  );
  try {
...
export const downloadFile = async (
  ...
  try {
    ...
    if (originalMaterialIdArr) {
      educationalmaterialId = parseInt(
        originalMaterialIdArr[0].educationalmaterialid,
        10
      );
    ...
    if (
      !req.isAuthenticated() ||
      !(await hasAccesstoPublication(educationalmaterialId, req))
    ) {
      try {
...
export const downloadFileFromStorage = async (
  ...
  winstonLogger.debug(
    "downloadFileFromStorage(): req.params.filename=" +
      req.params.filename +
      ", isZip=" +
      isZip
  );
  ...
  const fileName: string =
    (req.params.filename as string) || (req.params.key as string);
  return new Promise(async resolve => {
    try {
      ...
      if (!fileDetails) {
        next(
          new ErrorHandler(404, "Requested file " + fileName + " not found.")
        );
      } else {
        ...
        if (
          ...
          res.setHeader(
            "Location",
            config.STREAM_REDIRECT_CRITERIA.redirectUri + fileName
          );
          res.status(302);
      ...
      next(
        new ErrorHandler(
          500,
          "Downloading a single file failed in downloadFileFromStorage()"
        )
      );
    }
  });
 ...
 */
export async function readStreamFromStorage(params: {
  Bucket: string;
  Key: string;
}): Promise<any> {
  try {
      ...
      if (isZip) {
        ...
        const zipStream: WriteStream = fileStream
          .on("error", (error: Error) => {
            winstonLogger.error(
              "Error in zip file download in downloadFromStorage(): " + error
            );
            reject();
          ...
          .once("end", () => {
            winstonLogger.debug(
              "Download of %s completed in downloadFromStorage()",
              key
            );
          })
          .pipe(fs.createWriteStream(folderpath));
        ...
        fileStream
          .on("error", (error: Error) => {
            winstonLogger.error(
              "downloadFromStorage() - Error in single file download stream: " +
                error
            );
            reject();
...
export async function downloadMaterialFile(
  ...
  winstonLogger.debug(
    "downloadMaterialFile(): edumaterialid=" +
      req.params.edumaterialid +
      ", publishedat?=" +
  ...
  try {
    const versionFiles: {
      ...
      return await db.any(queryVersionFilesIds, [
        req.params.edumaterialid,
        publishedAt
      ]);
    });
    if (versionFiles.length < 1) {
      ...
      const educationalMaterialId: number = parseInt(
        req.params.edumaterialid,
        10
      );
      if (
...
export async function unZipAndExtract(zipFolder: any): Promise<any> {
  ...
  try {
    ...
    winstonLogger.debug(
      "The folderpath that came to the unZipandExtract function: " + zipFolder
    );
    // const filenameParsed = zipFolder.substring(0, zipFolder.lastIndexOf("/"));
    const filenameParsedNicely = zipFolder.slice(0, -4);
    winstonLogger.debug(
      "Hopefully the filename is parsed corectly: " + filenameParsedNicely
    );
    ...
    winstonLogger.debug(
      "The error in unzipAndExtract function for HTML zip: " + err
    );
    return false;
  }
}
