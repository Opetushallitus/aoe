...
export class AnalyticsViewComponent implements OnInit {
  ...
  selectOptionCategory: OptionCategory[] = [
    {
      key: 0,
      value: CategoryEnum.EDUCATIONAL_LEVEL,
      label: { fi: 'Opetusasteet', sv: '', en: '' }
    },
    {
      key: 1,
  ...
  updateFormFields(category: CategoryEnum): void {
    switch (category) {
      case CategoryEnum.EDUCATIONAL_LEVEL:
        this.formGroupPublished.addControl(
          'educationalLevels',
  ...
  submitFormUsage(buttonElement: HTMLButtonElement): void {
    ...
    if (this.formGroupUsage.valid) {
      ...
      Promise.all([
        ...
        this.getEditData()
      ]).then((response: EChartData[]): void => {
        this.chartData = response.filter((n: EChartData) => n)
        this.eChartsOptionActivity = this.setOptions(
          this.chartData,
          this.dateArray,
          'line'
        ) as EChartsOption
  ...
  submitFormExpired(buttonElement: HTMLButtonElement): void {
    ...
    const expiredBeforeString: string = this.statisticsService.dateToString(
      expiredBeforeDate,
      IntervalEnum.DAY
    )
    ...
    if (this.formGroupExpired.valid) {
      ...
      const educationalLevelNames: [{ key: string; value: string }] =
        this.formExpiredEducationalLevelsCtrl.value?.map(
  ...
  submitFormPublished(buttonElement: HTMLButtonElement): void {
    ...
    const startDateString: string = this.formPublishedDateSinceCtrl.value
      ? this.statisticsService.dateToString(
          new Date(this.formPublishedDateSinceCtrl.value),
          IntervalEnum.DAY
        )
      : null
    const endDateString: string = this.formPublishedDateUntilCtrl.value
      ? this.statisticsService.dateToString(
          new Date(this.formPublishedDateUntilCtrl.value),
          IntervalEnum.DAY
        )
      : null
    switch (this.formPublishedCategoryCtrl.value) {
    ...
    if (this.formGroupPublished.valid) {
      ...
      this.getSelectedClassifications()
        .then((categoryItems: { key: string; value: string }[]): void => {
          this.getPublishedMaterials(
            payload,
            this.formPublishedCategoryCtrl.value,
            categoryItems
          ).then((response: { portionNames: string[]; total: number[] }): void => {
          ...
          switch (interval) {
            case 'day':
              ...
              values.total = response.values.map(
                (value: IntervalResponse) => value.dayTotal
              ) as number[]
              resolve(values)
              break
            case 'week':
              values.dates = response.values.map(
                (value: IntervalResponse): string =>
                  `${value.year}-${String(value.week).padStart(2, '0')}`
              ) as string[]
              values.total = response.values.map(
