...
export class EducationalMaterialRatingModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      materialId: this.fb.control(this.materialId, [Validators.required]),
      ratingContent: this.fb.control(null, [Validators.min(1), Validators.max(5)]),
      ratingVisual: this.fb.control(null, [Validators.min(1), Validators.max(5)]),
      feedbackPositive: this.fb.control(null, [Validators.maxLength(1000)]),
      feedbackSuggest: this.fb.control(null, [Validators.maxLength(1000)]),
      feedbackPurpose: this.fb.control(null, [Validators.maxLength(1000)])
    })
    ...
    this.translate
      .get('forms.editEducationalResource.toasts.ratingAdded')
