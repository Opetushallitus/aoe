...
export class AddToCollectionModalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.userCollectionSubscription = this.collectionSvc.userCollections$.subscribe(
    ...
    if (this.newCollectionForm.valid) {
      if (this.newCollectionForm.dirty) {
        this.collectionSvc.createCollection(this.newCollectionForm.value).subscribe(() => {
          // show toast
          this.toastr.success(
            this.collectionCreatedToast.message,
            this.collectionCreatedToast.title
          )
    ...
    if (this.selectedCollections.length > 0) {
      this.selectedCollections.forEach((collection: number) => {
        ...
        this.collectionSvc
          ...
          .subscribe((response: AddToCollectionResponse) => {
            if (response.status === 'ok') {
              // show toast
              this.toastr.success(
                this.addedToCollectionToast.message,
                this.addedToCollectionToast.title
              )
