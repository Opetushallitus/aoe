 ...
 * (alignmentType, key, source) and also reports which were dropped. First
 * occurrence wins, so a pre-existing item is kept and every later near-duplicate
 * is dropped (in encounter order). Distinct from `deduplicate`, which keys on a
 * single property and keeps the last occurrence.
 * @param {AlignmentObjectExtended[]} items
 * @returns {{ deduped: AlignmentObjectExtended[]; dropped: AlignmentObjectExtended[] }}
 */
export function dedupeAlignmentObjectsWithDropped(items: AlignmentObjectExtended[]): {
  deduped: AlignmentObjectExtended[]
  dropped: AlignmentObjectExtended[]
} {
  if (!Array.isArray(items)) {
    return { deduped: items, dropped: [] }
  }
  const seen = new Set<string>()
  const deduped: AlignmentObjectExtended[] = []
  const dropped: AlignmentObjectExtended[] = []
...
/**
 * Removes alignment objects that collide on the DB unique key tuple
 * (alignmentType, key, source), keeping the first occurrence.
 * @param {AlignmentObjectExtended[]} items
...
export function dedupeAlignmentObjects(
  ...
  return dedupeAlignmentObjectsWithDropped(items).deduped
}
