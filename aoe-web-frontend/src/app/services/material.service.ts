...
export class MaterialService {
  ...
  private educationalMaterialID$$: BehaviorSubject<number | null> = new BehaviorSubject<
    number | null
  >(null)
  private uploadedFiles$$: BehaviorSubject<UploadedFile[] | null> = new BehaviorSubject<
    UploadedFile[] | null
  >(null)
  private uploadResponses$$: BehaviorSubject<UploadMessage[]> = new BehaviorSubject<
    UploadMessage[]
  >([])
  ...
  public publishedUserMaterials$$: Subject<EducationalMaterialCard[]> = new Subject<
    EducationalMaterialCard[]
  >()
  public unpublishedUserMaterials$$: Subject<EducationalMaterialCard[]> = new Subject<
    EducationalMaterialCard[]
  >()
  ...
  postLinks(data: LinkPost): Observable<UploadMessage> {
    return this.postLink(data, this.educationalMaterialID$$.getValue()).pipe(
      map(
        (
          response: any
        ): { message: number; status: string; visible: boolean; statusHTTP: number } => ({
          status: 'completed',
          message: 100,
          visible: true,
  ...
  uploadSingleFileToEducationalMaterial(data: FormData): Observable<UploadMessage> {
    ...
    return this.http
      ...
      .pipe(
        ...
        // timeout(3000),
        catchError((err) =>
          of({ status: 'error', message: prevProgress, visible: true, statusHTTP: err.status })
        )
      )
  }
          ...
          this.material$$.next({
            ...
            learningResourceTypes: material.learningResourceTypes.map(
              ({
                ...
                value
              }): { learningresourcetypekey: any; value: any } => ({
                learningresourcetypekey,
                value
              })
            ),
            authors: material.author.map(
              ({ authorname, organization }): { authorname: any; organization: any } => ({
                authorname,
                organization
              })
            ),
            ...
            keywords: material.keywords.map(
              ({ keywordkey, value }): { keywordkey: any; value: any } => ({
                keywordkey,
                value
              })
            ),
            ...
            educationalRoles: material.educationalRoles.map(
              ({
                educationalrolekey,
                educationalrole
              }): { educationalrole: any; educationalrolekey: any } => ({
                educationalrolekey,
                educationalrole
              })
            ),
            ...
            accessibilityFeatures: material.accessibilityFeatures.map(
              ({
                ...
                value
              }): { accessibilityfeaturekey: any; value: any } => ({
                accessibilityfeaturekey,
                value
              })
            ),
          ...
          const mappedMaterial: EducationalMaterialCard = {
            ...
            description:
              material.description.length > 0
                ? {
                    fi: material.description.find((row): boolean => row.language === 'fi')
                      .description,
                    sv: material.description.find((row): boolean => row.language === 'sv')
                      .description,
                    en: material.description.find((row): boolean => row.language === 'en')
                      .description
                  }
                : null,
            license: material.license,
  ...
  uploadImage(
    base64Image: string,
    educationalMaterialID?: number | string | null
   ...
   * @param {number} educationalMateriaID
   */
  updateEducationalMaterialEditForm(
    educationalMateriaID: number | string
            ...
            const editMaterial: EducationalMaterialForm = {
              name:
                educationalMaterial.name.length > 0
                  ? {
                      fi: educationalMaterial.name.find((name) => name.language === 'fi')
                        .materialname,
                      sv: educationalMaterial.name.find((name) => name.language === 'sv')
                        .materialname,
                      en: educationalMaterial.name.find((name) => name.language === 'en')
                        .materialname
                    }
                  : {
                      fi: null,
              ...
              thumbnail: educationalMaterial.thumbnail
                ? educationalMaterial.thumbnail.filepath
                : null,
              ...
              description:
                educationalMaterial.description.length > 0
                  ? {
                      fi: educationalMaterial.description.find((desc) => desc.language === 'fi')
                        .description,
                      sv: educationalMaterial.description.find((desc) => desc.language === 'sv')
                        .description,
                      en: educationalMaterial.description.find((desc) => desc.language === 'en')
                        .description
                    }
                  : {
                      fi: null,
              ...
              earlyChildhoodEducationObjectives: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.earlyChildhoodObjectives
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              preparatoryEducationSubjects: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.preparatoryEducationSubjects
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              preparatoryEducationObjectives: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.preparatoryEducationObjectives
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              upperSecondarySchoolObjectives: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.upperSecondaryObjectives
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              upperSecondarySchoolModulesNew: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.upperSecondaryModulesNew
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              upperSecondarySchoolObjectivesNew: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.upperSecondaryObjectivesNew
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              upperSecondarySchoolContentsNew: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) => alignment.source === koodistoSources.upperSecondaryContentsNew
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              newUpperSecondarySchoolFramework:
                upperSecondarySubjectsNew.length > 0 &&
              ...
              furtherVocationalQualifications: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) =>
                    alignment.source === koodistoSources.furtherVocationalQualifications
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
              ...
              specialistVocationalQualifications: educationalMaterial.educationalAlignment
                .filter(
                  (alignment) =>
                    alignment.source === koodistoSources.specialistVocationalQualifications
                )
                .map((alignment) => ({
                  key: alignment.objectkey,
                  source: alignment.source,
