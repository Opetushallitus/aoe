...
export class EditFilesComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.languageSubscription = this.koodistoService.languages$.subscribe((languages: Language[]): void => {
  ...
  onFileChange(event: Event, i: number): void {
    if ((event.target as HTMLInputElement).files.length > 0) {
      ...
      if (mimeTypes.video.includes(file.type)) {
        ...
        const subtitlesArray: FormArray = this.materialDetailsArray.at(i).get('subtitles') as FormArray;
  ...
  validateFiles(): void {
    this.materialDetailsArray.controls = this.materialDetailsArray.controls.filter((material: AbstractControl) => {
        ...
        subtitlesArray.controls.forEach((subtitle: AbstractControl): void => {
          if (!subtitle.get('subtitle').value && !subtitle.get('newSubtitle').value) {
            subtitlesArray.removeAt(
              subtitlesArray.controls.findIndex((subCtrl: AbstractControl): boolean => subCtrl === subtitle),
            );
          }
        });
      }
  ...
  calculateNewMaterialCount(): void {
    this.newMaterialCount = this.materialDetailsArray.controls.filter((material: AbstractControl) => {
  ...
  saveMaterial(): void {
    ...
    if (this.newMaterialCount > 0 || this.newSubtitleCount > 0) this.isVersioned = true;
  ...
  redirectToNextTab(): void {
    void this.router.navigate(['/muokkaa-oppimateriaalia', this.educationalMaterialID, this.tabId + 1]);
  }
