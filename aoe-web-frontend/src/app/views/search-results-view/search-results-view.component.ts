...
export class SearchResultsViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.searchFilterSubscription = this.searchService.searchFilters$.subscribe(
      (filters: SearchFilters) => {
        ...
        this.showAllOrganizations =
          this.organizationsArray.controls.length > this.filtersShownAtFirst
    ...
    this.sortCtrl.valueChanges.subscribe((value) =>
      this.sort2Ctrl.setValue(value, { emitEvent: false })
    )
    this.sort2Ctrl.valueChanges.subscribe((value) =>
      this.sortCtrl.setValue(value, { emitEvent: false })
    )
  }
  ...
  setAvailableFilters(searchFilters: SearchFilters): void {
    ...
    this.educationalLevels?.forEach((level: EducationalLevel, index: number) => {
      level.children.forEach((child: EducationalLevel, childIndex: number) => {
        ...
        levels
          .at(childIndex)
          .setValue(searchParams?.filters?.educationalLevels?.includes(child.key))
      })
    })
  ...
  onSubmit(removedFilter?: UsedFilter): void {
    if (this.searchForm.valid && this.filtersCount < 51) {
      ...
      this.educationalLevelsArray.value.forEach((level, index: number) => {
        ...
        switch (removedFilter.type) {
          ...
          case 'educationalSubjects':
            searchParams.filters.educationalSubjects =
              searchParams.filters.educationalSubjects.filter(
          ...
          case 'learningResourceTypes':
            searchParams.filters.learningResourceTypes =
              searchParams.filters.learningResourceTypes.filter(
          ...
          case 'hazard':
            searchParams.filters.accessibilityHazards =
              searchParams.filters.accessibilityHazards.filter(
                (hazard: string) => hazard !== removedFilter.key
              )
            break

          case 'feature':
            searchParams.filters.accessibilityFeatures =
              searchParams.filters.accessibilityFeatures.filter(
