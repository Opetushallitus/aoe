...
function create_tunnel {
 ...
 function wait_for_pg_isready {
    ...
    while ! pg_isready -h 127.0.0.1 -p ${SSH_TUNNEL_PORT} -U "${USERNAME}" -d "aoe" 2>/dev/null; do
      sleep 2
    done
  }

  function start_db_tunnel {
    docker-compose -f "${repo}/scripts/db/docker-compose.yml" up --build --detach --wait || {
      docker-compose -f "${repo}/scripts/db/docker-compose.yml" logs && exit 1
    }
    wait_until_port_is_listening "${SSH_TUNNEL_PORT}"
    docker compose -f "${repo}/scripts/db/docker-compose.yml" logs
  }
