...
export async function setOrganisaatiot(): Promise<void> {
  ...
  if (!results || !(results as any).organisaatiot || (results as any).organisaatiot.length < 1) {
    winstonLogger.error('No data for organisations from virkailija.opintopolku.fi in setOrganisaatiot()');
    return;
  }
  ...
  (results as any).organisaatiot.forEach((organisation: any) => {
    if (!organisation.oid || (!organisation.nimi?.fi && !organisation.nimi?.sv && !organisation.nimi?.en)) {
      throw Error('Creating new sets of organisations failed in setOrganisaatiot(): Missing required data');
    }
