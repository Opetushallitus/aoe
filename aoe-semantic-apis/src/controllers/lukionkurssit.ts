...
export async function setLukionkurssit(): Promise<void> {
  ...
  results.forEach((row: any) => {
    if (!row.metadata || !row.koodiArvo || !row.resourceUri) {
      throw Error('Creating new sets of courses failed in setLukionkurssit(): Missing required data');
    }
...
export const getLukionkurssi = async (
  ...
  try {
    ...
    if (redisData) {
      ...
      const row: AlignmentObjectExtended = input.find((e: any) => e.key.toLowerCase() === req.params.key.toLowerCase());
