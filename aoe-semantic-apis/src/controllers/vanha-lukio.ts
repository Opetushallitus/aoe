...
export async function setLukionVanhatOppiaineetKurssit(): Promise<void> {
  ...
  results.oppiaineet.forEach((subject: Oppiaine) => {
    if (!subject.koodiArvo || (!subject.nimi?.fi && !subject.nimi?.sv)) {
      throw Error('Creating new sets of subjects failed in setLukionVanhatOppiaineetKurssit(): Missing required data');
    }
    ...
    subject.kurssit?.forEach((course: Kurssi) => {
      if (!course.koodiArvo || (!course.nimi?.fi && !course.nimi?.sv)) {
        throw Error('Creating new sets of courses failed in setLukionVanhatOppiaineetKurssit(): Missing required data');
      }
...
export const getLukionVanhatOppiaineet = async (
  ...
  try {
    ...
    next(err);
    winstonLogger.error('Getting upper secondary school subjects failed in getLukionVanhatOppiaineet()');
  }
};
